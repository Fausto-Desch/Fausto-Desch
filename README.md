<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,100:2563eb&height=220&section=header&text=Fausto%20Desch&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=35"/>

# Fausto Desch

### Full Stack Developer | Técnico en Programación

Desarrollo de aplicaciones web modernas, escalables y enfocadas en rendimiento.

<p align="center">
  <a href="https://github.com/Fausto-Desch">
    <img src="https://img.shields.io/badge/GitHub-Fausto--Desch-181717?style=for-the-badge&logo=github">
  </a>

  <a href="mailto:fausdesch2210@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contactame-D14836?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
</p>

</div>

---

#  Sobre mí

Soy Técnico en Programación y desarrollador Full Stack con enfoque principal en tecnologías modernas del ecosistema JavaScript y TypeScript.

Me especializo en el desarrollo Front-End utilizando React y Next.js, creando interfaces modernas, optimizadas y escalables. Complemento mi perfil con conocimientos sólidos en backend, bases de datos, APIs REST y herramientas de desarrollo profesional.

Actualmente continúo perfeccionando mis habilidades técnicas mediante proyectos personales y colaborativos, priorizando siempre:

- Código limpio y mantenible
- Buenas prácticas de desarrollo
- Optimización de rendimiento
- Experiencia de usuario
- Escalabilidad de aplicaciones

---

#  Stack Tecnológico

## Frontend

<p>
  <img src="https://skillicons.dev/icons?i=html,css,tailwind,react,nextjs,ts" />
</p>

- HTML5
- CSS3
- Tailwind CSS
- React
- Next.js
- TypeScript

---

## Backend

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,python,cpp" />
</p>

- Node.js
- Python
- C++

---

## Bases de Datos

<p>
  <img src="https://skillicons.dev/icons?i=mongodb,mysql" />
</p>

- MongoDB
- MySQL

---

## Herramientas & DevOps

<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,vscode" />
</p>

- Git
- GitHub
- Docker
- VS Code

---

#  Áreas de Trabajo

- Desarrollo de aplicaciones SPA y SSR
- Arquitectura basada en componentes
- Creación y consumo de APIs REST
- Manejo de estado y tipado seguro
- Diseño responsive y UI moderna
- Optimización de performance y SEO
- Integración Frontend ↔ Backend
- Control de versiones y trabajo colaborativo

---

#  Objetivos Profesionales

Actualmente busco oportunidades como:

- Front-End Developer
- Full Stack Developer
- Freelancer Web Developer
- Colaborador en proyectos tecnológicos

Interesado en trabajar en proyectos desafiantes donde pueda seguir creciendo profesionalmente y aportar valor técnico al equipo.

---

#  Estadísticas de GitHub

<div align="center">

<a href="https://github.com/Fausto-Desch">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=Fausto-Desch&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true"/>
</a>

<a href="https://github.com/Fausto-Desch">
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Fausto-Desch&layout=compact&theme=tokyonight&hide_border=true"/>
</a>

<br><br>

<img src="https://streak-stats.demolab.com?user=Fausto-Desch&theme=tokyonight&hide_border=true"/>

</div>

---

#  Actualmente aprendiendo

- Arquitectura escalable con Next.js
- Backend avanzado con Node.js
- Docker y despliegue de aplicaciones
- Buenas prácticas de Clean Code
- Optimización y performance web

---

#  Contacto

 Email:  
**fausdesch2210@gmail.com**

 GitHub:  
 https://github.com/Fausto-Desch

 Instagram:  
 https://www.instagram.com/xfaustodeschx/


---

<div align="center">

###  Gracias por visitar mi perfil

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:2563eb,100:0f172a&height=120&section=footer"/>

</div>
